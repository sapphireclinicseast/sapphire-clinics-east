  ...
  try {
    ...
    const totalKg = items.reduce((sum, item) => {
      const product = item.slug
        ? getProductBySlug(item.slug)
        : allProducts.find((p) => p.id === item.productId)
      // Per-variant weight when the selected variant carries one (SMALL vs LARGE
      // ship in different boxes); otherwise the product's weight.
      const variant = product?.variants?.find(
        (v) =>
          (item.variantSku && v.sku === item.variantSku) ||
          (item.variantLabel && v.label === item.variantLabel),
      ...
      const w =
        variant && typeof variant.weightKg === 'number' && variant.weightKg > 0
          ? variant.weightKg
          : product?.weightKg
