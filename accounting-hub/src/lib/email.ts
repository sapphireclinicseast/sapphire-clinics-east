// Outbound email via the Gmail API (see lib/gmail.ts). SMTP ports are blocked
// on the VPS, so anything SMTP-based hangs — this goes over HTTPS.
//
// Investor letters send as main@sapphireclinicseast.org, which is now the
// authenticated mailbox rather than a relay pretending to be it.
...
export async function sendInvestorEmail(opts: { to: string; subject: string; html: string; attachments?: MailAttachment[] }): Promise<{ ok: boolean; from: string; error?: string }> {
  // Gmail sends as the authenticated mailbox, so the old two-step dance
  // (try main@, fall back to a verified relay with reply-to) is gone — main@
  // IS the sending mailbox, which is what these letters always wanted to be.
  ...
  return { ok: res.ok, from: res.from, error: res.error }
}
