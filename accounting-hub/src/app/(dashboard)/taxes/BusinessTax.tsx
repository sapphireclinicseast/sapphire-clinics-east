...
const BRANCH_FULL: Record<string, string> = { ALL: 'Sapphire Clinics East Inc. (all branches)', SBEA: 'Aura Health Rehab — East', SBGH: 'Aura Health Rehab — Greenhills', VERDANA: 'Verdana Store' }
...
export default function BusinessTax() {
  ...
  const fetchSummary = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/taxes/vat-summary?payrollBranch=${branch === 'ALL' ? '' : branch}&from=${from}&to=${to}`)
  ...
  const fetchRfps = useCallback(async () => {
    try { const res = await fetch(`/api/taxes/rfp?taxType=VAT${branch === 'ALL' ? '' : `&payrollBranch=${branch}`}`); setRfps(res.ok ? await res.json() : []) } catch { setRfps([]) }
  }, [branch])
