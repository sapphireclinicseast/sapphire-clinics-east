...
export default function ExpensesPage() {
  ...
  // "Add to Asset Management" prompt for one-time entries tagged with a PPE classification.
  ...
  return (
    <div className={expanded ? 'fixed inset-0 z-50 overflow-auto p-6 space-y-4' : 'space-y-4'} style={expanded ? { background: 'var(--off-white)' } : undefined}>
                  ...
                  {displayed.map(e => {
                    ...
                    return (
                      <tr key={e.id} style={{ background: e.paidAt ? '#dcfce7' : e.reimbursementId ? '#ffedd5' : e.soaId ? '#f3e8ff' : e.finalized ? '#fef9c3' : '#fff' }}>
                        ...
                        <td className="border-b px-2 py-1 text-center align-middle" style={{ borderColor: 'var(--light-gray)', position: 'sticky', right: 0, zIndex: 4, background: '#fff', boxShadow: '-5px 0 6px -4px rgba(0,0,0,0.18)', minWidth: 132 }}>
      ...
      {assetPrompt && (() => {
        ...
        const targets = [{ branch: assetPrompt.branch, price: num(assetPrompt.grossAmount) * vf }]
        return (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={() => setAssetPrompt(null)}>
            <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm mb-3" style={{ color: 'var(--mid-gray)' }}>
                This one-time expense is classified as <strong>{cls} — {ASSET_CLASSIFICATION_LABELS[cls]}</strong>.
