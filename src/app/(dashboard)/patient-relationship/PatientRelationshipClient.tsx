...
import {
  ...
  Upload, Eye, Trash2, FileText, QrCode, Loader2, X, Banknote, RotateCcw, CalendarCheck,
} from 'lucide-react'

type Tab = 'waitlist' | 'followup' | 'cancellation' | 'noshow'
...
function CancellationTab({ branch }: { branch: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg p-3 text-xs space-y-1" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', color: '#92400E' }}>
        <p>Each patient is allowed a maximum of <strong>2 cancellations</strong> per 6-month window (from first session date). Yellow = 1 remaining, Red = none remaining.</p>
        <p>At <strong>12 total cancellations/retainers</strong> (all time), the patient is subject for <strong>slot removal</strong>.</p>
      </div>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: '#fff', border: '1px solid var(--light-gray)' }}>
        <table className="w-full text-left" style={{ fontSize: '0.82rem' }}>
          <thead>
            <tr style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
              {['', 'Patient', 'Branch', 'Window (6mo)', 'Total'].map(h => (
                <th key={h} className="px-4 py-3 font-semibold" style={{ color: 'var(--mid-gray)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {paginated.length === 0 ? (
              ...
              const slotRemoval = (p.totalCancellations || 0) >= 12
              const rowColor = slotRemoval ? '#FDF2F8' : p.cancellationsRemaining === 0 ? '#FEF2F2' : p.cancellationsRemaining === 1 ? '#FFFBEB' : undefined
              const textColor = slotRemoval ? '#9D174D' : p.cancellationsRemaining === 0 ? '#DC2626' : p.cancellationsRemaining === 1 ? '#D97706' : 'var(--charcoal)'
              return (
                <Fragment key={p.id}>
                  <tr style={{ borderBottom: '1px solid #F3F4F6', cursor: 'pointer', background: rowColor }}
                    ...
                    <td className="px-4 py-3 font-semibold" style={{ color: textColor }}>
                      ...
                      {slotRemoval && <span className="ml-2 text-xs px-1.5 py-0.5 rounded-full" style={{ background: '#9D174D', color: '#fff' }}>SLOT REMOVAL</span>}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm font-bold" style={{ color: slotRemoval ? '#9D174D' : 'var(--charcoal)' }}>{p.totalCancellations || 0}</span>
                            ...
                            <div className="space-y-2">
                              {p.logs.map((log: any) => (
                                <div key={log.id} className="rounded-lg p-3 flex flex-wrap items-center gap-3 text-xs"
                                  style={{
                                    background: log.madeUp ? '#F0FDF4' : log.deletedAt ? '#F9FAFB' : '#fff',
                                    border: `1px solid ${log.madeUp ? '#BBF7D0' : log.deletedAt ? '#E5E7EB' : '#E2E8F0'}`,
                                    opacity: log.deletedAt ? 0.6 : 1,
                                  }}>
                                  <div className="flex-1 min-w-0">
                                    <span className="font-semibold px-2 py-0.5 rounded-full" style={{
                                      background: log.madeUp ? '#DCFCE7' : log.isValid ? '#F0FDF4' : '#FEF2F2',
                                      color: log.madeUp ? '#166534' : log.isValid ? '#15803D' : '#DC2626',
                                    }}>{log.type}</span>
                                    <span className="ml-2" style={{ color: '#6B7280' }}>
                                      {new Date(log.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                    </span>
                                    {log.madeUp && (
                                      <span className="ml-2 text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: '#22C55E', color: '#fff' }}>
                                        MADE UP
                                      </span>
                                    )}
                                  ...
                                  <div className="flex gap-1.5 items-center flex-wrap">
                                    {/* Make Up Session button — not deleted, not already made up */}
                                    {!log.deletedAt && !log.madeUp && (
                                      <button onClick={async (e) => { e.stopPropagation(); if (!confirm('Mark this cancellation as having a make-up session on the same month? This will remove it from the cancellation count.')) return; await fetch('/api/patient-relationship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'mark-makeup-cancellation', logId: log.id }) }); fetchData() }}
                                        ...
                                        title="Did a Make Up Session on same month">
                                        <CalendarCheck size={12} /> Make Up Session
                                      </button>
                                    )}
                                    {/* Fee buttons — shown for patients beyond 2/2 and active logs */}
                                    {p.cancellationsRemaining === 0 && !log.deletedAt && !log.madeUp && (
                                      <>
                                        {!log.feeDeducted ? (
                                          <button onClick={async (e) => { e.stopPropagation(); await fetch('/api/patient-relationship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'deduct-fee-cancellation', logId: log.id }) }); fetchData() }}
                                            className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1"
                                            style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FDE68A', cursor: 'pointer' }}>
                                            <Banknote size={12} /> Deducted from Downpayment
                                          </button>
                                        ) : (
                                          <span className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1" style={{ background: '#F0FDF4', color: '#15803D', border: '1px solid #BBF7D0' }}>
                                            <CheckCircle size={12} /> Fee Deducted
                                          </span>
                                        )}
                                        {log.feeDeducted && !log.feePaidOnReturn ? (
                                          <button onClick={async (e) => { e.stopPropagation(); await fetch('/api/patient-relationship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'paid-return-cancellation', logId: log.id }) }); fetchData() }}
                                            className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1"
                                            style={{ background: '#DBEAFE', color: '#1E40AF', border: '1px solid #93C5FD', cursor: 'pointer' }}>
                                            <RotateCcw size={12} /> Paid upon Return
                                          </button>
                                        ) : log.feePaidOnReturn ? (
                                          <span className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1" style={{ background: '#F0FDF4', color: '#15803D', border: '1px solid #BBF7D0' }}>
                                    ...
                                    {/* Upload proof (only for valid, no proof yet, not deleted, not made up) */}
                                    {log.isValid && !log.proofUrl && !log.deletedAt && !log.madeUp && (
                                      <>
                                    ...
                                    {/* Delete (soft) — hidden for made-up logs */}
                                    {!log.deletedAt && !log.madeUp && (
...
function NoShowTab({ branch }: { branch: string }) {
  ...
  async function saveLog() {
    if (!logForm) return
    setLogSaving(true)
    const b = branch || patients.find((p: any) => p.id === logForm.patientId)?.branch || 'SANDBOX_EAST'
    await fetch('/api/patient-relationship', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'create-noshow', patientId: logForm.patientId, branch: b, remarks: logForm.remarks || undefined }),
    })
    setLogForm(null)
    setLogSaving(false)
    fetchData()
  ...
  async function deleteLog() {
    ...
    await fetch(`/api/patient-relationship?${params}`, { method: 'DELETE' })
    setDeleting(null)
    fetchData()
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg p-3 text-xs" style={{ background: '#FEF2F2', border: '1px solid #FECACA', color: '#991B1B' }}>
        Patients with <strong>3 or more no-shows</strong> are subject for <strong>slot removal</strong>. Yellow = 2 no-shows, Red = 3+.
      </div>
        ...
        <table className="w-full text-left" style={{ fontSize: '0.82rem' }}>
          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              const isExpanded = expandedId === p.id
              const slotRemoval = p.noShowCount >= 3
              const rowColor = slotRemoval ? '#FEF2F2' : p.noShowCount === 2 ? '#FFFBEB' : undefined
              const textColor = slotRemoval ? '#DC2626' : p.noShowCount === 2 ? '#D97706' : 'var(--charcoal)'
              return (
                <Fragment key={p.id}>
                  <tr style={{ borderBottom: '1px solid #F3F4F6', cursor: 'pointer', background: rowColor }}
                    onClick={() => setExpandedId(isExpanded ? null : p.id)}>
                    ...
                    <td className="px-4 py-3 font-semibold" style={{ color: textColor }}>
                      {p.lastName}, {p.firstName}
                      {slotRemoval && <span className="ml-2 text-xs px-1.5 py-0.5 rounded-full" style={{ background: '#DC2626', color: '#fff' }}>SLOT REMOVAL</span>}
                    </td>
                    <td className="px-4 py-3" style={{ color: '#6B7280' }}>{branchLabel(p.branch)}</td>
                    <td className="px-4 py-3">
                      <span className="text-sm font-bold" style={{ color: textColor }}>{p.noShowCount}</span>
                      <span className="text-xs ml-1" style={{ color: '#9ca3af' }}>/ 3</span>
                    </td>
                  </tr>
                  {isExpanded && (
                    <tr key={p.id + '-detail'}>
                        ...
                        <div className="space-y-3">
                          ...
                          {p.logs.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {p.logs.map((log: any) => (
                                <div key={log.id} className="rounded-lg p-3 flex flex-wrap items-center gap-3 text-xs"
                                  style={{ background: log.deletedAt ? '#F9FAFB' : '#fff', border: `1px solid ${log.deletedAt ? '#E5E7EB' : '#E2E8F0'}`, opacity: log.deletedAt ? 0.6 : 1 }}>
                                  <div className="flex-1 min-w-0">
                                    <span className="font-semibold" style={{ color: '#DC2626' }}>No Show</span>
                                    <span className="ml-2" style={{ color: '#6B7280' }}>
                                      {new Date(log.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                    </span>
                                    {log.remarks && <span className="ml-2" style={{ color: '#9ca3af' }}>- {log.remarks}</span>}
                                  ...
                                  <div className="flex gap-1.5 items-center flex-wrap">
                                    {!log.deletedAt && (
                                      <>
                                        {!log.feeDeducted ? (
                                          <button onClick={async (e) => { e.stopPropagation(); await fetch('/api/patient-relationship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'deduct-fee-noshow', logId: log.id }) }); fetchData() }}
                                            className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1"
                                            style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FDE68A', cursor: 'pointer' }}>
                                            <Banknote size={12} /> Deducted from Downpayment
                                          </button>
                                        ...
                                        {log.feeDeducted && !log.feePaidOnReturn ? (
                                          <button onClick={async (e) => { e.stopPropagation(); await fetch('/api/patient-relationship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'paid-return-noshow', logId: log.id }) }); fetchData() }}
                                            className="text-xs px-2 py-1 rounded-lg font-semibold flex items-center gap-1"
                                            style={{ background: '#DBEAFE', color: '#1E40AF', border: '1px solid #93C5FD', cursor: 'pointer' }}>
                                            <RotateCcw size={12} /> Paid upon Return
                                          </button>
