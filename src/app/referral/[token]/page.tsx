...
export default function ReferralUploadPage() {
  ...
  useEffect(() => {
    fetch(`/api/referral-upload/${token}`)
      .then(r => r.json())
      .then(d => {
  ...
  async function upload() {
    ...
    const res = await fetch(`/api/referral-upload/${token}`, { method: 'POST', body: form })
      ...
      <div style={card}>
        ...
        {stage === 'error' && (
          <div style={{ textAlign: 'center', padding: '1rem 0' }}>
            ...
            <p style={{ fontWeight: 700, color: charcoal, marginBottom: '0.5rem' }}>
              Link Unavailable
            </p>
        ...
        {stage === 'ready' && !selectedFile && (
          <>
            ...
            <p style={{ color: '#666', fontSize: '0.875rem', marginBottom: '1.75rem', lineHeight: 1.5 }}>
              {hasExisting
                ? 'A referral is already on file. Upload a new one to replace it.'
                : "Please upload your doctor's referral document below."}
            </p>
        ...
        {stage === 'ready' && selectedFile && (
          <>
            <h2 style={{ fontWeight: 700, color: charcoal, marginBottom: '1rem', fontSize: '1rem' }}>
        ...
        {stage === 'success' && (
          <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
            ...
            <p style={{ fontWeight: 700, color: charcoal, fontSize: '1.1rem', marginBottom: '0.5rem' }}>
              Document Uploaded!
            </p>
            <p style={{ color: '#555', fontSize: '0.875rem', lineHeight: 1.5 }}>
              Your doctor&apos;s referral has been saved to your patient record.
