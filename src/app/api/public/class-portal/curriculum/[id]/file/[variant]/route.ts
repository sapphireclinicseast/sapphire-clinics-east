// GET /api/public/class-portal/curriculum/[id]/file/[variant]
//   Streams the stored bytes for a curriculum variant (pdf | doc | xls).
//   Any authenticated class-portal user can download.
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string; variant: string }> }) {
  ...
  try {
    ...
    if (variant !== 'pdf' && variant !== 'doc' && variant !== 'xls') {
      return withCors(NextResponse.json({ error: 'variant must be pdf | doc | xls.' }, { status: 400 }), origin)
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const row = await (prisma.classPortalCurriculum as any).findUnique({
      where: { id },
      select: {
        pdfFileName: true, pdfFileType: true, pdfFileData: true,
        docFileName: true, docFileType: true, docFileData: true,
        xlsFileName: true, xlsFileType: true, xlsFileData: true,
      },
    })
    if (!row) {
    ...
    console.error('[curriculum/file.GET]', e)
