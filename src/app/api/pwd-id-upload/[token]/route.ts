...
// POST → accept file upload, update patient pwdIdUrl, mark token used
...
async function resolveToken(token: string) {
  const record = await prisma.pwdIdUploadToken.findUnique({
    where: { token },
    include: { patient: { select: { id: true, firstName: true, pwdIdUrl: true } } },
  })
...
export async function GET(
  ...
  return NextResponse.json({
    ...
    hasExisting: !!result.record.patient.pwdIdUrl,
  })
}
...
export async function POST(
  ...
  const filename = `pwdid-${patientId}-${Date.now()}${ext}`
  const uploadDir = path.join(process.cwd(), 'uploads')
  await fs.mkdir(uploadDir, { recursive: true })
  await fs.writeFile(path.join(uploadDir, filename), Buffer.from(await file.arrayBuffer()))

  if (record.patient.pwdIdUrl) {
    const oldFile = record.patient.pwdIdUrl.split('/api/uploads/')[1]
  ...
  await prisma.$transaction([
    prisma.patient.update({ where: { id: patientId }, data: { pwdIdUrl } }),
    prisma.pwdIdUploadToken.update({ where: { id: record.id }, data: { used: true } }),
  ])

  return NextResponse.json({ ok: true, pwdIdUrl })
}
