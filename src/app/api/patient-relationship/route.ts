...
const MAX_TOTAL_CANCELLATIONS = 12
...
export async function GET(req: NextRequest) {
  ...
  if (tab === 'cancellation') {
    ...
    const results = patients.map(p => {
      ...
      // Count active (non-deleted, non-made-up) cancellation logs within window
      const activeLogs = p.cancellationLogs.filter(l => !l.deletedAt)
      const countableLogs = activeLogs.filter(l => !l.madeUp) // made-up sessions don't count
      const windowLogs = windowStart
        ? countableLogs.filter(l => {
      ...
      const totalCancellations = countableLogs.length
      ...
      return {
        ...
        totalCancellations,
        subjectForSlotRemoval: totalCancellations >= MAX_TOTAL_CANCELLATIONS,
        logs: p.cancellationLogs.map(l => ({
          ...
          madeUp: l.madeUp,
          madeUpAt: l.madeUpAt,
          feeDeducted: l.feeDeducted,
          feeDeductedAt: l.feeDeductedAt,
          feePaidOnReturn: l.feePaidOnReturn,
          feePaidAt: l.feePaidAt,
  ...
  if (tab === 'noshow') {
    ...
    const results = patients.map(p => {
      ...
      const noShowCount = activeLogs.length
      ...
      return {
        ...
        branch: p.branches?.[0] || p.branch,
        noShowCount,
        subjectForSlotRemoval: noShowCount >= MAX_NO_SHOWS,
  ...
  // ── NOTIFICATIONS (for front desk dashboard) ────────────────
  if (tab === 'notifications') {
    // 1. Follow-up nearing (within 30 days of due)
    const followUpNearing: any[] = []
    const now = new Date()
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate())

    for (const [dept, interval] of Object.entries(DEPT_FOLLOWUP)) {
      const schedules = await prisma.schedule.findMany({
        where: {
          staff: { department: dept },
          patient: branchFilter ? { ...branchFilter } : undefined,
      ...
      for (const s of schedules) {
        if (!s.patientId || !s.patient) continue
        if (!patientMap.has(s.patientId)) {
          patientMap.set(s.patientId, { patient: s.patient, firstSession: s })
        }
      }

      for (const { patient: p, firstSession } of patientMap.values()) {
        const refDate = p.doctorConsultDate ? new Date(p.doctorConsultDate) : new Date(firstSession.date)
        const daysSince = Math.floor((today.getTime() - refDate.getTime()) / (1000 * 60 * 60 * 24))
        const dueIn = interval.days - daysSince
        if (dueIn >= 0 && dueIn <= 30) {
          followUpNearing.push({
            id: p.id,
            firstName: p.firstName,
            lastName: p.lastName,
            department: dept,
    ...
    // 2. Cancellation alerts (12+ total)
    const cancPatients = await prisma.patient.findMany({
      where: branchFilter,
      include: { cancellationLogs: true },
    })
    const cancellationAlerts = cancPatients
      .map(p => {
        const active = p.cancellationLogs.filter(l => !l.deletedAt).length
        return { id: p.id, firstName: p.firstName, lastName: p.lastName, totalCancellations: active }
      })
      .filter(p => p.totalCancellations >= MAX_TOTAL_CANCELLATIONS)

    // 3. No-show alerts (3+)
    const nsPatients = await prisma.patient.findMany({
      where: branchFilter,
      include: { noShowLogs: true },
    })
    const noShowAlerts = nsPatients
      .map(p => {
        const active = p.noShowLogs.filter(l => !l.deletedAt).length
        return { id: p.id, firstName: p.firstName, lastName: p.lastName, noShowCount: active }
      })
      .filter(p => p.noShowCount >= MAX_NO_SHOWS)

    return NextResponse.json({ followUpNearing, cancellationAlerts, noShowAlerts })
  }
...
export async function POST(req: NextRequest) {
  ...
  // ── Fee actions ──
  if (action === 'deduct-fee-noshow') {
    await prisma.noShowLog.update({ where: { id: body.logId }, data: { feeDeducted: true, feeDeductedAt: new Date() } })
    return NextResponse.json({ ok: true })
  }
  if (action === 'paid-return-noshow') {
    await prisma.noShowLog.update({ where: { id: body.logId }, data: { feePaidOnReturn: true, feePaidAt: new Date() } })
    return NextResponse.json({ ok: true })
  }
  if (action === 'deduct-fee-cancellation') {
    await prisma.cancellationLog.update({ where: { id: body.logId }, data: { feeDeducted: true, feeDeductedAt: new Date() } })
    return NextResponse.json({ ok: true })
  }
  if (action === 'paid-return-cancellation') {
    await prisma.cancellationLog.update({ where: { id: body.logId }, data: { feePaidOnReturn: true, feePaidAt: new Date() } })
    return NextResponse.json({ ok: true })
  }
  if (action === 'mark-makeup-cancellation') {
    await prisma.cancellationLog.update({ where: { id: body.logId }, data: { madeUp: true, madeUpAt: new Date() } })
    return NextResponse.json({ ok: true })
  }

  // ── Create no-show log ──
  if (action === 'create-noshow') {
    const { patientId, branch, remarks } = body
    if (!patientId || !branch) return NextResponse.json({ error: 'patientId and branch required' }, { status: 400 })
    const log = await prisma.noShowLog.create({ data: { patientId, branch, remarks } })
    return NextResponse.json({ ok: true, log })
  }

  // ── Create cancellation log (default / backward-compatible) ──
...
export async function DELETE(req: NextRequest) {
  ...
  if (tab === 'noshow-delete') {
    ...
    await prisma.noShowLog.update({
      where: { id: logId },
      data: { deletedAt: new Date(), deletedBy: user?.name || user?.email || 'Unknown', deleteReason: reason },
    })
    return NextResponse.json({ ok: true })
  }
