...
function RSAText({ track, name, program, school, monthly, months }: { track?: string; name: string; program: string; school: string; monthly?: number | null; months?: number | null }) {
  ...
  const tail = (
    <>
      ...
      <p><b>6.1 Cash Buyout.</b> Should the FELLOW elect not to render, or be unable to complete, the Return Service Obligation, it may be discharged through a cash payment equal to the number of unrendered hours multiplied by an Hourly Credit Rate (PHP 150.00 per hour), plus a flat eight percent (8%) surcharge. By way of illustration, the buyout for the full 1,500 hours equals PHP 225,000.00 plus the 8% surcharge, totaling PHP 243,000.00.</p>
      ...
      <div className={s.rsaScroll}>
        ...
        {isTindig ? (
          ...
          <>
            ...
            <p><b>2.4 Cut-Short of Internship.</b> Should the FELLOW be unable to continue or complete the internship for any reason, the Allowance ceases as of the date of discontinuance, and the FELLOW and CO-MAKER agree to reimburse the Allowance actually received plus an eight percent (8%) surcharge within ninety (90) days. Full reimbursement finally settles the Parties&rsquo; obligations. SCEI may, with compassion, waive, reduce, or restructure the reimbursement — particularly for serious illness, a death in the immediate family, or circumstances beyond the FELLOW&rsquo;s control.</p>
...
function ScholarAcceptance({ scholar, token, authHeaders }: { scholar: PortalScholar; token: string; authHeaders: Record<string, string> }) {
  ...
  return (
    <div className={s.sec} style={{ padding: 0 }}>
      <RSAText track={scholar.track} name={fullName} program={scholar.program} school={scholar.school} monthly={scholar.awardMonthly} months={scholar.awardMonths} />
      {msg && <div className={`${s.alert2} ${msg.ok ? s.alertOk2 : s.alertErr2}`}>{msg.t}</div>}
