...
// ── Options from the OT Daily Notes PDF ─────────────────────────────────────
...
const EXECUTION_LEVELS = ['Independent', 'With Supervision', 'With Assistance', 'Dependent']
const PARTICIPATION_LEVELS = ['Independent', 'With Supervision', 'With Assistance', 'Dependent']
...
export interface OTFormData {
  ...
  activitiesAndPerformance: string
  assessment: {
    taskExecution: string
    overallParticipation: string
    didWellIn: string
    needsImprovementIn: string
  }
  plan: {
    ...
    <div className="flex flex-wrap gap-3">
      {options.map((opt) => (
        <label key={opt} className="flex items-center gap-2 cursor-pointer group">
          <input type="radio" name={name} checked={selected === opt} onChange={() => onChange(opt)}
            className="accent-[var(--teal)] w-4 h-4" />
          <span className="text-[13px] text-[var(--charcoal)] group-hover:text-[var(--deep-teal)]">{opt}</span>
        </label>
      ))}
    </div>
  )
...
export default function OTNoteForm({ patientName, sessionDate, onSubmit, submitting, onCancel, initialData, clinicianSettings }: Props) {
  ...
  const [activitiesAndPerformance, setActivitiesAndPerformance] = useState(init?.activitiesAndPerformance ?? '')

  const [taskExecution, setTaskExecution] = useState(init?.assessment?.taskExecution ?? '')
  const [overallParticipation, setOverallParticipation] = useState(init?.assessment?.overallParticipation ?? '')
  const [didWellIn, setDidWellIn] = useState(init?.assessment?.didWellIn ?? '')
  const [needsImprovementIn, setNeedsImprovementIn] = useState(init?.assessment?.needsImprovementIn ?? '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const data: OTFormData = {
      formType: 'OT_DAILY_NOTES',
      subjective: { sessionType, sessionTypeOther },
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      ...
      <div className="text-center mb-4">
        <span className="inline-block px-3 py-1 rounded-full text-[11px] font-bold tracking-wider uppercase bg-orange-100 text-orange-700 mb-2">
          OT Daily Notes
        </span>
      ...
      <CollapsibleSection title="Subjective">
        <p className="text-[13px] text-[var(--mid-gray)] mb-3 italic">
          <strong>{patientName}</strong> was seen and managed today for occupational therapy.
        </p>
      ...
      {/* Section 2: Objective */}
      <CollapsibleSection title="Objective — The following were targeted">
      ...
      <CollapsibleSection title="Activities and Performance">
        <textarea value={activitiesAndPerformance} onChange={(e) => setActivitiesAndPerformance(e.target.value)}
          className="input text-[13px] min-h-[120px] resize-y"
          placeholder="Describe activities performed and patient's performance..." />
      </CollapsibleSection>
      ...
      <CollapsibleSection title="Assessment">
        <div className="space-y-4">
          <div>
            <label className="block text-[12px] font-semibold text-[var(--charcoal)] uppercase tracking-wider mb-2">Task/Skills Execution</label>
            <RadioGroup name="taskExecution" options={EXECUTION_LEVELS} selected={taskExecution} onChange={setTaskExecution} />
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-[var(--charcoal)] uppercase tracking-wider mb-2">Overall Participation</label>
            <RadioGroup name="overallParticipation" options={PARTICIPATION_LEVELS} selected={overallParticipation} onChange={setOverallParticipation} />
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-[var(--charcoal)] uppercase tracking-wider mb-1">Did well in</label>
            <textarea value={didWellIn} onChange={(e) => setDidWellIn(e.target.value)}
              className="input text-[13px] min-h-[80px] resize-y" placeholder="Areas the patient excelled in..." />
          </div>
          <div>
            <label className="block text-[12px] font-semibold text-[var(--charcoal)] uppercase tracking-wider mb-1">Needs improvement in</label>
            <textarea value={needsImprovementIn} onChange={(e) => setNeedsImprovementIn(e.target.value)}
              className="input text-[13px] min-h-[80px] resize-y" placeholder="Areas needing improvement..." />
          </div>
        </div>
      </CollapsibleSection>
