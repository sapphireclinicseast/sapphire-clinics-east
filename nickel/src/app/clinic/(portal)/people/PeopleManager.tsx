...
export default function PeopleManager({ verified, patients, providers }: { verified: boolean; patients: Pt[]; providers: Pr[] }) {
  ...
  const [pf, setPf] = useState({ firstName: '', lastName: '', email: '', phone: '', address: '', password: '' })
  const [tf, setTf] = useState({ firstName: '', lastName: '', email: '', phone: '', profession: 'PT', rate: '', password: '' })
  ...
  async function createPatient() {
    ...
    try {
      ...
      setPf({ firstName: '', lastName: '', email: '', phone: '', address: '', password: '' }); setMsg('Patient account created.'); router.refresh()
  ...
  async function createTherapist() {
    ...
    try {
      ...
      const d = await r.json(); if (!r.ok) throw new Error(d.error ?? 'Failed')
      setTf({ firstName: '', lastName: '', email: '', phone: '', profession: 'PT', rate: '', password: '' }); setMsg('Therapist account created.'); router.refresh()
    } catch (e) { setMsg(e instanceof Error ? e.message : 'Failed') } finally { setBusy(false) }
  }
