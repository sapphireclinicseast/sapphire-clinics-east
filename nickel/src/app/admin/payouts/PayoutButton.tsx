...
export default function PayoutButton({ providerId, amount }: { providerId: string; amount: number }) {
  ...
  async function pay() {
    ...
    try {
      const r = await fetch('/api/admin/payout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ providerId, reference: ref }) })
