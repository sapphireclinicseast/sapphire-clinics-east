...
export default function DoctorAccountActions({ deactivated }: { deactivated: boolean }) {
  const router = useRouter()
  const [busy, setBusy] = useState<string | null>(null)
  const [msg, setMsg] = useState<string | null>(null)

  async function act(action: 'deactivate' | 'reactivate' | 'delete') {
    if (action === 'deactivate' && !confirm('Deactivate your account? You’ll be hidden from patients and won’t receive new consults. You can reactivate anytime.')) return
    ...
    try {
      const r = await fetch('/api/doctor/account', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) })
      const d = await r.json(); if (!r.ok) throw new Error(d.error ?? 'Failed')
      if (action === 'delete') { window.location.href = '/doctor/login'; return }
      router.refresh()
  ...
  return (
    <section className="card space-y-3">
      ...
      {deactivated ? (
        <>
          <p className="text-[12.5px] text-amber-800">Your account is <b>deactivated</b> — you’re hidden from patients. Reactivate to appear again.</p>
        ...
        <>
          <p className="text-[12px] text-[color:var(--slate)]">Take a break or leave Nickel. Deactivating hides you from patients (reversible). Deleting removes you from the platform.</p>
