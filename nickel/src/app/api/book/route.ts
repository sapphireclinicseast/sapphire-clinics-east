...
export async function POST(req: NextRequest) {
  ...
  const b = (await req.json().catch(() => ({}))) as { providerId?: string; date?: string; startTime?: string; city?: string }
  ...
  // Reserve the slot transactionally (guards against a double-book race).
  ...
  try {
    booking = await prisma.$transaction(async (tx) => {
      ...
      return tx.booking.create({
        data: { patientId, providerId, city, date: bookedDate, startTime, endTime: addHour(startTime), amount, transpoIncluded: provider.transpoIncluded, status: 'PENDING' },
        select: { id: true },
      })
    })
  } catch (e) {
  ...
  try {
    const link = await createPaymongoLink({
      ...
      remarks: provider.transpoIncluded ? 'Transportation included' : 'Transportation not included',
    })
